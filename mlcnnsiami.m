...
% 		end
	...
	function [pred] = test(self,data,costFun)
	...
	%respectively.
	%
	% Returns a mlcnnsiami object, <net>.
	%--------------------------------------------------------------------------
	...
	%--------------------------------------------------------------------------
		...
		fileName = fullfile(self.saveDir,sprintf('mlcnnsiami.mat'));
		net = self.bestNet;
		save(fileName,'net');
	end
end % END METHODS
